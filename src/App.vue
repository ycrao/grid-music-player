...
// 核心状态定义
// ========================================
const audio = ref(null)                    // <audio> 元素引用
const playlist = ref([])                   // 播放列表数组
const currentIndex = ref(-1)               // 当前播放歌曲索引
const currentLyrics = ref([])              // 当前歌曲的歌词数组 [{t: 时间, v: 文本}]
const lyricPointer = ref(0)                // 当前歌词行指针
const lastRenderedPointer = ref(-1)        // 上一次渲染的歌词行索引

// DOM 引用数组（每首歌对应一个格子）
const lyricRefs = []   // 歌词容器
const vizRefs = []     // 可视化 canvas
const barRefs = []     // 进度条

// 动态设置 ref（Vue 无法直接传索引到模板，故用函数）
...
const tileClass = i => ({
  ...
  shrink:    currentIndex.value !== -1 && currentIndex.value !== i,
...
function tileStyle(i) {
  const p = colRow(i)
  return {
    gridColumn: basePos[i]?.c ?? p.col,
    gridRow:    basePos[i]?.r ?? p.row
  }
}

// 判断格子是否应被“折叠”（被当前播放格子覆盖）
function isCollapsed(i) {
  if (currentIndex.value < 0) return false
  const t = colRow(currentIndex.value)
  const r0 = t.row <= 2 ? t.row : t.row - 1
  const c0 = t.col <= 3 ? t.col : t.col - 1
  const p = colRow(i)
...
async function loadPlaylist() {
  try {
    const res = await fetch('/res/playlist.json')
    const items = await res.json()
    playlist.value = normalizePaths(items)
  } catch (e) {
    // 加载失败时使用默认歌曲
    playlist.value = normalizePaths([{
      title: '九万字',
      artist: '黄诗扶',
...
function normalizePaths(items) {
  return items.map(it => {
    const fix = p => !p ? p : (p.startsWith('/') ? p : `/${p}`)
    return { ...it, audio: fix(it.audio), lyrics: fix(it.lyrics), cover: fix(it.cover) }
  })
}
...
function expandTile(i) {
  resetLayout()
  const p = colRow(i)
  const r0 = p.row <= 2 ? p.row : p.row - 1
  const c0 = p.col <= 3 ? p.col : p.col - 1
  basePos[i] = {
    c: `${c0} / span 2`,
    r: `${r0} / span 2`
  }
...
function ensureLyricContainer() {
  ...
  return lyricRefs[currentIndex.value]
}

function ensureBar() {
  if (currentIndex.value < 0) return null
  return barRefs[currentIndex.value]
}
...
function placeOverlayChar(container, line) {
  ...
  const candidates = [
    { x: cx, y: lr.top - cr.top - M },
    { x: cx, y: lr.bottom - cr.top + M },
    { x: lr.left - cr.left - M, y: cy },
    { x: lr.right - cr.left + M, y: cy },
    { x: lr.left - cr.left - M, y: lr.top - cr.top - M },
    { x: lr.right - cr.left + M, y: lr.top - cr.top - M },
    { x: lr.left - cr.left - M, y: lr.bottom - cr.top + M },
    { x: lr.right - cr.left + M, y: lr.bottom - cr.top + M }
  ]
  ...
  if (!choice) {
    choice = {
      x: Math.min(cr.width - size, Math.max(size, cx)),
      y: Math.min(cr.height - size, Math.max(size, cy - size - M))
    }
  }
...
function hideInactive() {
  for (let k = 0; k < count; k++) {
    if (k === currentIndex.value) continue
    ...
    if (v) {
      ...
      ctx && ctx.clearRect(0, 0, v.width, v.height)
...
function startVisualizer() {
  ensureAudio()
  vizCanvas = vizRefs[currentIndex.value]
  if (!vizCanvas) return
  ...
  function draw() {
    ...
    for (let i = 0; i < bins; i++) {
      ...
      const x = (i / (bins - 1)) * w
      const amp = v * h * 0.45
      const y1 = baseline - amp
      const y2 = baseline + amp * 0.06

      ctx.beginPath()
      ctx.moveTo(x, y1)
      ctx.lineTo(x, y2)
      ctx.strokeStyle = `hsla(${(vizHue + i * 4) % 360},88%,62%,.95)`
      ctx.lineWidth = 2.4
      ctx.stroke()
    ...
    energy /= 22
    vizHue = (vizHue + 1 + energy * 7) % 360

    const now = audioCtx.currentTime
    beatEnv = beatEnv * 0.9 + energy * 0.1
    const delta = energy - beatEnv

    if (delta > 0.12 && now - lastBeat > 0.18) {
      lastBeat = now
      setPulse(energy, true)
      spawnBeatGlow()
    } else {
      setPulse(energy, false)
    }
...
async function playIndex(i) {
  ...
  audio.value.src = playlist.value[i].audio
  currentLyrics.value = await loadLyrics(playlist.value[i].lyrics)
  lyricPointer.value = 0
  lastRenderedPointer.value = -1
...
function updateLyric(force) {
  ...
  // 前进指针到当前时间
  while (
    lyricPointer.value + 1 < currentLyrics.value.length &&
    currentLyrics.value[lyricPointer.value + 1].t <= ct
  ) {
    lyricPointer.value++
  }

  if (!force && lyricPointer.value === lastRenderedPointer.value) return

  lastRenderedPointer.value = lyricPointer.value
  const container = ensureLyricContainer()
  if (!container) return

  const now = currentLyrics.value[lyricPointer.value]
  container.innerHTML = ''

  if (now) {
    const l = buildLine(now.v, true)
    l.style.setProperty('--rot', `${(Math.random() * 6 - 3).toFixed(1)}deg`)
    container.appendChild(l)
    placeOverlayChar(container, l)
  }
}

// 节奏脉冲效果
function setPulse(level, beat) {
  const el = ensureLyricContainer()
  if (!el) return

  const line = el.querySelector('.line')
  if (!line) return

  pulsePrev = pulsePrev * 0.8 + level * 0.2
  const boost = beat ? 0.45 : 0.0
  const val = (1 + pulsePrev * 0.9 + boost).toFixed(3)
  line.style.setProperty('--pulse', val)

  if (beat) {
    line.style.setProperty('--rot', `${(Math.random() * 10 - 5).toFixed(1)}deg`)
  }
}

// 节拍闪光
function spawnBeatGlow() {
  const tile = lyricRefs[currentIndex.value]?.parentElement
  if (!tile) return

  const g = document.createElement('div')
  g.className = 'beat-glow'
  tile.appendChild(g)
...
function prev() {
  ...
  const j = currentIndex.value <= 0 ? playlist.value.length - 1 : currentIndex.value - 1
  playIndex(j)
}

function next() {
  if (!playlist.value.length) return
  const j = (currentIndex.value + 1) % playlist.value.length
  playIndex(j)
}

function toggle() {
  if (!audio.value.src && playlist.value.length) {
    playIndex(0)
  } else {
...
onMounted(async () => {
  ...
  // 可视化器控制
  audio.value.addEventListener('pause', stopVisualizer)
  audio.value.addEventListener('play', startVisualizer)
})
</script>